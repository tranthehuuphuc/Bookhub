...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($conn->query($sql) === TRUE) {
        ...
        $sql_info = "INSERT INTO book_information (book_id, pages, language, format, isbn, chapters)
                     VALUES ('$book_id', '$pages', '$language', '$format', '$isbn', '$chapters')";
        $conn->query($sql_info);

        // Insert book-category associations into the database
        foreach ($categories as $category_id) {
            $sql_category = "INSERT INTO book_category (book_id, category_id) VALUES ('$book_id', '$category_id')";
